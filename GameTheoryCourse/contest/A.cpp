...
void solve() {
    ...
    for (auto &num : moves) cin >> num;

    sort(all(moves));
#ifdef DEBUG

    for (auto &num : moves) cout << num << " ";
    ...
    for (auto &num : rocks) cout << num << " ";

    cout << endl;
#endif
    cout << ((result) ? ("Gana el primer jugador.") : ("Gana el segundo jugador."))
         << endl;
}
