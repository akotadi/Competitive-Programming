...
VI greatestPrime, divisors;

void greatestPrimeSieve() {
    greatestPrime.resize(MAXN, 1);
    greatestPrime[0] = greatestPrime[1] = 0;

    for (int i = 2; i < MAXN; i++) greatestPrime[i] = i;
...
void divisorsSieve() {
    divisors.resize(MAXN, 1);
    divisors[2] = divisors[3] = 2;

    for (int i = 4; i < MAXN; ++i) {
        int aux = i, counter = 1;

        while (aux % greatestPrime[i] == 0) {
            counter++;
            aux /= greatestPrime[i];
        }

        divisors[i] = divisors[aux] * counter;
    }
}
...
void solve(int caseNumber) {
    ...
    cout << "Case #" << caseNumber + 1 << ": " << (n - divisors[n] - ((
                n & 1) ? (1) :
            (0))) << endl;
}
