...
void solve(){
    ...
    n = readNumber();
    a.resize(2, VI(n, 0));
    dp.resize(2, vector<Long> (n, -1));
    for (int i = 0; i < n; ++i) a[0][i] = readNumber();
    for (int i = 0; i < n; ++i) a[1][i] = readNumber();
    ...
    if (n > 2)
    {
        dp[0][n-1] = a[0][n-1];
        dp[1][n-1] = a[1][n-1];
        dp[0][n-2] = a[0][n-2] + dp[1][n-1];
        dp[1][n-2] = a[1][n-2] + dp[0][n-1];
        for (int i = n-3; i >= 0; --i)
        {
        ...
        result = max(a[0][0] + a[1][1], a[0][1] + a[1][0]);
    }else{
        result = max(a[0][0], a[1][0]);
    }
