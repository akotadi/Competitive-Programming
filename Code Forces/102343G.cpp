...
bool isValid(VS &matrix, int x, int y) {
    if (x < 0 || x >= n)
        return false;
    ...
    return matrix[x][y] == '.' || matrix[x][y] == 'F';
}

int recursive(VS &matrix, bool turn, PII lastC, PII lastB, int counter) {
    if (lastC == target && lastB == target)
        return counter;

    if (lastC == target || !turn) {
        if (dp[lastB.fi][lastB.se] != 0x3f3f3f3f)
            return dp[lastB.fi][lastB.se];

        int check = INT_MAX;

        for (int i{}; i < 4; i++) {
            int x = lastB.fi + movX[i], y = lastB.se + movY[i];

            if (isValid(matrix, x, y)) {
                VS aux = matrix;
                aux[x][y] = (matrix[x][y] == 'F') ? ('F') : ('B');
                check = min(check, recursive(aux, true, lastC, {x, y}, counter + 1));
            }
        }

        dp[lastB.fi][lastB.se] = check;
    } else {
        if (dp[lastC.fi][lastC.se] != 0x3f3f3f3f)
            return dp[lastC.fi][lastC.se];

        int check = INT_MAX;

        for (int i{}; i < 4; i++) {
            int x = lastC.fi + movX[i], y = lastC.se + movY[i];

            if (isValid(matrix, x, y)) {
                VS aux = matrix;
                aux[x][y] = (matrix[x][y] == 'F') ? ('F') : ('C');
                check = min(check, recursive(aux, false, {x, y}, lastB, counter + 1));
            }
        }

        dp[lastC.fi][lastC.se] = check;
    }
}
