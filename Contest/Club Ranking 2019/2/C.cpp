...
void solve(){
    Long min = calculate(1), max = calculate(calculate(1));
    int size = MAXM;
    while (size && min != max) {
        min = calculate(min);
        max = calculate(calculate(max));
        size--;
    }
    if (size <= 0) writeNumber(-1);
    else{
        int index = 0;
        min = 1;
        while (min != max) {
...
int main(){
    ...
    a = readNumber<Long>(), b = readNumber<Long>(), c = readNumber<Long>(); 
