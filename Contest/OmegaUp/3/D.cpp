...
void solve(){
    ...
    PII ind = {0,0};
    int dir = 0;
    while(isValid(ind.fi, ind.se)){
        if(ind.fi == n-1 && ind.se == m-1){
            cout << "llegaste" << endl;
            return;
        }
        if(a[ind.fi][ind.se] == 1)
            dir = (dir + 1) % 4;
        else if(a[ind.fi][ind.se] == 2)
            dir = (dir - 1 + 4) % 4;
        else if(a[ind.fi][ind.se] == 3)
            dir = (dir + 2) % 4;
        ind = {ind.fi + movs[dir].fi, ind.se + movs[dir].se};
    }
