...
struct Grafo {
    int n;
    unordered_map<Long, unordered_set<Long> > adj, usedEdges;
    ...
    int BFS(Long s, Long t) {
        ...
        while (!q.empty()) {
            ...
            for (Long u : adj[v]) {
                if (usedEdges[u].find(v) != usedEdges[u].end()) continue;
                if (!used[u]) {
                    used[u] = true;
                    q.push(u);
                    d[u] = d[v] + 1;
    ...
    int findMinCycle() {
        ...
        for (auto& it : adj) {
            ...
            // while (!it.se.empty()) {
                ...
                usedEdges[x].insert(y);
                usedEdges[y].insert(x);

                result = min(result, BFS(x, y));
            }
        }
...
void solve(){
    ...
    for (auto& num1 : a) {
        for(auto& num2 : a) {
            ...
            if (num1 & num2) g.addEdge(num1, num2);
        }
    }

    writeNumber(g.findMinCycle());
