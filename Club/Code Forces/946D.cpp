...
void solve(){
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (int j = 0; j <= k; ++j)
        {
            ...
            auto auxFirst = s.find_first_of('0', first + 1);
            auto auxLast = s.find_last_of('0', last - 1);

            if (last - s.find_first_of('1', first + 1) <= s.find_last_of('1', last - 1) - first)
            {
                first = s.find_first_of('1', first + 1);
            } else {
                last = s.find_last_of('1', last - 1);
            }
        }
    }
    ...
    for (int i = n-2; i >= 0; --i)
    {
        for (int j = 0; j <= k; ++j)
        {
            for (int first = 0, last = j; first <= last; ++first, --last)
            {
                c[i][j] = min(c[i][j], b[i][first] + c[i+1][last]);
                c[i][j] = min(c[i][j], b[i][last] + c[i+1][first]);
            }
        }
    }
